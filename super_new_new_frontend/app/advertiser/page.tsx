...
export default function RegisterAdvertiser() {
  ...
  const handleRegister = async () => {
    if (vector.every(v => v === 0)) { setStatus("Set at least one category weight above 0."); return; }
    ...
    <div className="min-h-screen bg-[#030303] text-white font-sans">
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        <div className="space-y-8">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <label className="block text-sm font-medium text-white/80 tracking-wide uppercase">
                Target Interest Weights
              </label>
              <span className="text-xs font-mono text-white/30">Total: {vector.reduce((a,b)=>a+b,0)}/500</span>
            </div>
            <div className="space-y-4">
              {categories.map((cat, i) => (
                <div key={cat} className="flex items-center gap-4">
                  <span className={`text-[11px] font-mono uppercase w-16 ${
                    vector[i] > 0 ? 'text-white/70' : 'text-white/20'
                  }`}>{cat}</span>
                  <div className="flex-1 relative">
                    <input
                      ...
                      style={{
                        background: `linear-gradient(to right, rgba(255,255,255,0.8) 0%, rgba(255,255,255,0.8) ${vector[i]}%, rgba(255,255,255,0.08) ${vector[i]}%, rgba(255,255,255,0.08) 100%)`
                      }}
                    />
                  </div>
                  <input
                    ...
                    className="w-14 bg-white/[0.04] text-white text-center text-sm font-mono py-1 rounded-lg border border-white/10 focus:outline-none focus:border-white/30 transition-colors"
                  />
                </div>
              ))}
            </div>
            {/* Vector preview */}
            <div className="mt-4 px-3 py-2 bg-black rounded-lg border border-white/[0.06] flex items-center gap-2">
              <span className="text-[10px] text-white/20 font-mono">vec:</span>
              <div className="flex gap-1.5">
                {vector.map((v, i) => (
                  <span key={i} className={`text-xs font-mono px-1.5 py-0.5 rounded ${
                    v > 50 ? 'bg-white/15 text-white' : v > 0 ? 'bg-white/[0.06] text-white/50' : 'text-white/15'
                  }`}>{v}</span>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-white/[0.03] border border-white/10 rounded-xl p-6">
              ...
              <p className="text-xs text-white/30 mt-3">Max payout per impression<br/><span className="text-white/20">bid × (matchScore / maxScore)</span></p>
            </div>
            <div className="bg-white/[0.03] border border-white/10 rounded-xl p-6">
